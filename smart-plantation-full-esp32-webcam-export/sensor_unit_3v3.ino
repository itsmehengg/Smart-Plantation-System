...
const char* MQTT_CLIENT_ID = "esp32_sensor_unit";
...
void publishSensorData() {
  ...
  if (isnan(temperature) || isnan(humidity)) {
    Serial.println("DHT read failed. Sending analog sensors only.");
    temperature = -999;
    humidity = -999;
  }
  ...
  snprintf(
    payload,
    sizeof(payload),
    "{\"temperature\":%.1f,\"humidity\":%.1f,\"soil_moisture\":%d,\"soil_digital\":%d,\"water_level\":%d,\"water_distance_cm\":%.1f,\"soil_raw\":%d}",
    temperature,
    humidity,
    soilMoisture,
    soilDigital,
  ...
  mqtt.publish(MQTT_SENSOR_TOPIC, payload);
