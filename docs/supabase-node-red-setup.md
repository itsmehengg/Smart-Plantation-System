...
If it works, `Supabase HTTP status` should show:

```text
201
```

and `Supabase insert result` should show the inserted row from Supabase.

If it fails with permission/RLS error, rerun the SQL file and make sure the insert policy was created.

If `Raw sensor payload` does not show anything, Node-RED is not receiving MQTT from the ESP32 yet. Check that the ESP32 Serial Monitor prints JSON and that both ESP32 and Node-RED use:

```text
Broker: broker.hivemq.com
Topic: smartplantation/sensors
```
